...
void MainProcess(TCanvas* can, TH1F* h_data, TH1F* h_mc, TString h_title, TString pdf_name, TString name_legend_bunshi, TString name_legend_bunbo ){
  ...
  for ( int ibinX = 0; ibinX < nbinX+1; ibinX++ ) {
    if(ibinX != nbinX+1){
      ...
      binX[ibinX] = h_data->GetBinWidth(ibinX) + h_data->GetBinLowEdge(ibinX);
    }
  }
  ...
  //Draw legend
  SetLegend(h_data, h_mc, name_legend_bunshi, name_legend_bunbo);
