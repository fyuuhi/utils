    ...
    m_jobOptionsSvc(0), m_trigCompositeContainer(0)
{
  ...
  declareProperty("UseFTKRoad",m_use_ftkroad = true);
  declareProperty("FTKAlgo", m_ftkalgo="InDetTrigTrackingxAODCnv_Muon_FTKRefit");
                                          ...
                                          HLT::TriggerElement* outputTE)
{
  ...
  if(m_use_ftkroad) {
    HLT::ErrorCode ecftk = getFeature(outputTE, idtracks, m_ftkalgo);
    if (ecftk != HLT::OK) {
      ATH_MSG_DEBUG(" Failed to get " << m_ftkalgo << " xAOD::TrackParticleContainer --> ABORT");
      return ecftk;
    }
  }
  if(!idtracks) {
    ATH_MSG_DEBUG(" " << m_ftkalgo << " container is empty");
  }
  ...
  for (p_roi=muonRoIs.begin(); p_roi!=muonRoIs.end(); ++p_roi) {
    ...
    if(m_use_ftkroad && idtracks) {
      ATH_MSG_DEBUG("FTK size=" << idtracks->size());
      for (xAOD::TrackParticleContainer::const_iterator trkit = idtracks->begin(); trkit != idtracks->end(); ++trkit) {
        ATH_MSG_DEBUG("FTK eta/phi=" << (*trkit)->eta() << "/" << (*trkit)->phi());
      }
    }
    cout << "hello1" << endl;
    ATH_MSG_DEBUG("muonColl->size(): " << muonColl->size() );

    std::vector<int> vecMuonRoadAlgo;


    if (idtracks && (idtracks->size() > 0.)) { // FTK
      for (xAOD::TrackParticleContainer::const_iterator trkit = idtracks->begin(); trkit != idtracks->end(); ++trkit) {
        ATH_MSG_DEBUG("FTK eta/phi=" << (*trkit)->eta() << "/" << (*trkit)->phi());
        ATH_MSG_DEBUG("muonColl->size(): " << muonColl->size() );
        ...
        if ( m_recMuonRoIUtils.isBarrel(*p_roi) ) { // Barrel
          ...
          if (!sc.isSuccess()) {
            ...
            updateOutputTE(outputTE, inputTE, muonColl, *p_roi, *p_roids, m_muonRoad, m_mdtRegion, m_rpcHits, m_tgcHits,
        ...
        vecMuonRoadAlgo.push_back(m_muonRoad.roadAlgo);
      ...
      const xAOD::TrackParticle* trkit_tmp = 0;
